...
from pyspark.sql.types import StructType, DoubleType, BooleanType
...
def transform_flights(df_raw: DataFrame) -> DataFrame:
    ...
    # Get the schema of the exploded 'flight' column
    flight_fields = df.schema["flight"].dataType.fields
    live_field = next((f for f in flight_fields if f.name == "live"), None)
    is_live_struct = live_field and isinstance(live_field.dataType, StructType)
    ...
    if is_live_struct:
        ...
        select_exprs.extend(
            [
                lit(None).cast(DoubleType()).alias("live_latitude"),
                lit(None).cast(DoubleType()).alias("live_longitude"),
                lit(None).cast(DoubleType()).alias("live_altitude"),
                lit(None).cast(BooleanType()).alias("live_is_ground"),
            ]
        )
    ...
    df_clean = (
        df_flat.withColumn("dep_scheduled_ts", to_timestamp("dep_scheduled"))
